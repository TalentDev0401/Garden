...
public class SKTiledSceneCamera: SKCameraNode {
    ...
    public func setCameraZoom(scale: CGFloat) {
        setCameraZoom(scale)
    }
    ...
    public func fitToView(newSize: CGSize, transition: TimeInterval = 0, verbose: Bool = false) {
        ...
        let scaleFactor = usableWidth / mapsize.width// (isPortrait == true) ?  : usableHeight / mapsize.height

        //let heightOffset: CGFloat = (usableHeight / 20)
        let focusPoint = CGPoint(x: tilemapCenter.x, y: tilemapCenter.y)
                
        setCameraZoom(scaleFactor, interval: 0.0)
    ...
    */
    ...
    @objc public func sceneDoubleTapped(_ recognizer: UITapGestureRecognizer) {
        ...
        if (recognizer.state == UIGestureRecognizer.State.ended && allowPause) {
            ...
            let location = recognizer.location(in: recognizer.view)
            focusLocation = scene.convertPoint(fromView: location)
            let maxScale = UIScreen.screenWidth / (512 * (SharedInstance.shared.matrix! + 0.3))
            let stepScale: CGFloat = 0.2
            if self.zoom < maxScale {
                self.zoom += stepScale
            } else {
                if (self.zoom + stepScale) < (maxScale + 0.3) {
                    self.zoom += 0.2
                } else {
                    self.zoom = maxScale
                }
            }
            setCameraZoomAtLocation(scale: self.zoom, location: CGPoint(x: 0, y: 0))
        }
    }
    ...
    @objc public func scenePinched(_ recognizer: UIPinchGestureRecognizer) {
        ...
        if recognizer.state == .began {
            ...
            focusLocation = scene.convertPoint(fromView: location)
            // calls `cameraPositionChanged`
            centerOn(scenePoint: CGPoint(x: 0, y: 0))//focusLocation
        }

        if recognizer.state == .changed {
            zoom *= recognizer.scale
            // set the world scaling here
            setCameraZoomAtLocation(scale: zoom, location: CGPoint(x: 0, y: 0))
            recognizer.scale = 1
        }
    }
}
